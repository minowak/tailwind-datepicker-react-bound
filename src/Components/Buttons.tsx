...
export const ButtonPrevMonth = () => {
  ...
  return (
    <button
      ...
      onClick={() => changeSelectedDate("prev", new Date(goToPrevNext(view, currentDate, -1)))}
    >
      {options?.icons?.prev ? (
        options?.icons?.prev()
      ) : (
...
export const ButtonSelectMonth = () => {
  ...
  return (
    ...
    >
      {view === "days" &&  getFormattedDate(currentDate, { month: "long", year: "numeric" })}
      {view === "months" && getFormattedDate(currentDate, { year: "numeric" })}
      {view === "years" &&
        `${startOfYearPeriod(currentDate, 10)}-${startOfYearPeriod(currentDate, 10) + 9}`}
      {view === "decades" &&
        `${startOfYearPeriod(currentDate, 100)}-${startOfYearPeriod(currentDate, 100) + 90}`}
    </button>
  );
};
...
export const ButtonNextMonth = () => {
  ...
  return (
    <button
      ...
      onClick={() => changeSelectedDate("next", new Date(goToPrevNext(view, currentDate, 1)))}
    >
      {options?.icons?.next ? (
        options?.icons?.next()
      ) : (
